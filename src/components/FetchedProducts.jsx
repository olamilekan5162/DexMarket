    ...
    <div className="flex flex-row flex-wrap container items-center mt-6 px-2 gap-4 justify-center">
      {
        prod.map((product) =>{
          ...
          const discountedPrice = product.price * (discount / 100)
          return(
          <div key={product.id} className="flex flex-col space-y-2 h-fit w-[300px] rounded pb-6 border hover:shadow-md">
                    ...
                    <p>
                        <span className='line-through text-red-700'>$-{discountedPrice.toFixed(2)}</span> 
